...
export default function TeacherQuizDetailPage() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (res.ok) router.push("/dashboard/teacher/quizzes")
    } finally {
      setDeleting(false)
    }
  }
            ...
            quiz.questions.map((q, qi) => (
              ...
              >
                ...
                <div className="mb-4 flex items-start gap-3">
                  ...
                  <p className="pt-0.5 text-base font-semibold leading-snug text-gray-800">
                    {q.question}
                  </p>
                </div>
                  ...
                  {q.options.map((opt, oi) => (
                    ...
                    >
                      <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                        opt.isCorrect ? "bg-emerald-500 text-white" : "bg-gray-200 text-gray-600"
                      }`}>
                        {OPTION_LABELS[oi]}
                      </div>
                      <span className={`text-sm font-medium ${opt.isCorrect ? "text-emerald-700" : "text-gray-600"}`}>
                        {opt.text}
                      </span>
                      {opt.isCorrect && (
