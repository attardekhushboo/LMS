...
export async function GET() {
  try {
    ...
    // Now seed fresh demo users with properly hashed password 'demo123'
    const demoUsers = [
      {
    ...
    for (const user of demoUsers) {
      try {
        ...
        `
        if (result.length > 0) {
          results.push(`✅ Reset & created user: ${user.email} (${user.role})`)
        }
      } catch (userError) {
    ...
    return NextResponse.json({
      success: true,
      message: 'Demo users reset and seeded successfully',
      results,
    })
  } catch (error) {
